...
export class DossiersParlementairesExtractor implements IExtractor {
    ...
    private extractData(data: any): void {
        ...
        const dossierObj = {
            ...
            titre_chemin: extractNilableValue(
                dossier.titreDossier?.titreChemin
            ),
            senat_chemin: extractNilableValue(
                dossier.titreDossier?.senatChemin
            ),

            procedure_code: extractNilableValue(
                dossier.procedureParlementaire?.code
            ),
            procedure_libelle: extractNilableValue(
        ...
        this.extractInitiateurs(
            dossier.uid,
            dossier.initiateur
        );

        const actes = this.asArray(
    ...
    private extractInitiateurs(
        ...
        const acteurs = this.asArray(
            initiateur.acteurs?.acteur
        );

        const organes = this.asArray(
            initiateur.organes?.organe
        );
        ...
        const obj = {
            ...
            acteur_uid:
                acteur?.acteurRef ?? null,

            mandat_uid:
                acteur?.mandatRef ?? null,

            organe_uid:
                organe?.organeRef?.uid ??
                organe?.organeRef ??
                null,
    ...
    private extractRapporteurs(
        ...
        const rapporteurs = this.asArray(
            acte.rapporteurs?.rapporteur
        );
    ...
    private extractDecision(
        ...
        const obj = {
            ...
            famille_code:
                extractNilableValue(
                    acte.statutConclusion?.fam_code
                ),
