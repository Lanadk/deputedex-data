...
// ==============================================================================
// AMENDEMENTS IMPORT JOB
// Import des JSON amendements pour chaque législature
// Raw → Snapshot → Final avec cleanup optionnel
// Port TS de amendements-import.sh
// ==============================================================================
...
const SCRIPTS_DIR = path.join(srcDir, "sql", "scripts", "amendements");
...
async function main(): Promise<void> {
    ...
    logger.info("==============================================");
    logger.info("  AMENDEMENTS IMPORT JOB");
    logger.info("==============================================");

    const steps: Step[] = [
        { kind: "sqlFile", label: "[SCHEMA] Import schema", path: path.join(SCHEMA_DIR, "amendements.schema.sql") },
    ];
    ...
    for (const legislature of legislatures) {
        ...
        steps.push(
            {
                kind: "importRaw",
                label: `[RAW ${legislature}] amendements`,
                jsonFile: path.join(dir, "amendements.json"),
                rawTable: "amendements_raw",
                projectionSqlFile: path.join(SCRIPTS_DIR, "projections/project_amendements.sql"),
            },
            {
                kind: "importRaw",
                label: `[RAW ${legislature}] co-auteurs`,
                jsonFile: path.join(dir, "amendementsCoAuteurs.json"),
                rawTable: "amendements_co_auteurs_raw",
                projectionSqlFile: path.join(SCRIPTS_DIR, "projections/project_amendements_co_auteurs.sql"),
            }
        );
    }
