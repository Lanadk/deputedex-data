...
$$
    ...
    BEGIN
        ...
        FROM (SELECT data ->> 'row_hash'
              FROM documents_co_signataires_raw
              GROUP BY 1
              HAVING COUNT(*) > 1) t;

        IF duplicate_count > 0 THEN
            RAISE NOTICE 'Detected % duplicate documents_co_signataires row_hash(es)', duplicate_count;
        ELSE
            RAISE NOTICE 'No duplicate documents_co_signataires row_hash detected';
        END IF;
    END
$$;
...
INSERT
INTO documents_co_signataires_snapshot (document_uid,
                                        acteur_uid,
                                        organe_uid,
                                        row_hash,
